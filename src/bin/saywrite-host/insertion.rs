...
async fn clipboard_outcome(backend: Backend, tool: &str, args: &[&str], text: &str) -> Result<InsertionOutcome> {
...
async fn try_backend(backend: Backend, text: &str) -> Result<InsertionOutcome> {
    match backend {
        ...
        Backend::WlClipboard => clipboard_outcome(backend, "wl-copy", &[], text).await,
        Backend::Xclip => clipboard_outcome(backend, "xclip", &["-selection", "clipboard"], text).await,
        Backend::Xsel => clipboard_outcome(backend, "xsel", &["--clipboard", "--input"], text).await,
        Backend::NotifySend => {
